---
title: 
author: 
date:
output: 
  revealjs::revealjs_presentation:
    css: metropolis.css
    self_contained: true
    transition: none
    background_transition: none
    reveal_options:
      slideNumber: true
      previewLinks: true
      minScale: 1.0
      maxScale: 1.0
      fig_caption: true
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
```

```{r, echo=FALSE}
library(visNetwork)
```

## Konzeptpräsentation 

<div id="left">
  <font size="10">**Bronzezeit, Bestattungen & Cultural Evolution**</font>

  Clemens Schmid \
  *28. Februar 2018*
</div>

<div id="right">
  <figure>
    <img src="figures/bronce_age_burial_reconstruction.jpg" alt="The Pulpit Rock">
    <figcaption>© Mike Codd, hampshirearchaeology.wordpress.com [13.02.18]</figcaption>
  </figure>
</div>

## Bestattungsbräuche der europäischen Bronzezeit

<div class="fragment" data-fragment-index="1">
- Gräber und Gräberfelder sind eine wesentliche Kategorie der Bronzezeitforschung
</div>

\

<div class="fragment" data-fragment-index="2">
- räumliche und zeitliche **Heterogenität der Rituale**
     - Grabanlage- und Vergesellschaftung (Nachnutzung neolithischer Megalithik, Gräberfelder, etc.) 
     - Grabbau (Särge, Totenhäuser, Bootsgräber, etc.)
     - Beigabenauswahl 
     - Platzierung des Leichnams
     - ...
</div>

\

<div class="fragment" data-fragment-index="3">
- **Hauptspannungsfelder**
    - Körperbesttattung vs. Brandbestattung
    - Flachgrab vs. Hügelgrab
</div>
    
## Klassisches Narrativ der Entwicklung bronzezeitlicher Bestattungssitten

<div class="fragment" data-fragment-index="1">
**Frühe und Mittlere Bronzezeit**

- Körperbestattungen (in verschiedenen Variationen)
- Brandgräber dominieren zunächst nur in der Ungarischen Tiefebene
</div>
<div class="fragment" data-fragment-index="2">
- Hügelgräber in Ost-, West- und Nordeuropa
- Flachgräber in Zentral- und Südeuropa, Überhügelung gewinnt an Bedeutung 
</div>

\

<div class="fragment" data-fragment-index="3">
**Späte Bronzezeit**

- Brandbestattung setzt sich (fast) überall durch: *Urnenfelderzeit*
</div>

## Quantitative Erfassung der Phänomene: RADON-B

<figure>
  <img src="figures/radonb.jpg" alt="radon-b" align="middle">
</figure>

<div class="fragment" data-fragment-index="1">
<sup>14</sup>C-Daten mit Kontextinformationen

- X Daten aus einem Zeitfenster 2500 -- 500 calBC
</div>
<div class="fragment" data-fragment-index="2">
- Metainformationen
     - X Daten daraus mit einer Angabe zu Körpergrab/Brandgrab
     - X Daten daraus mit einer Angabe zu Flachgrab/Hügelgrab
     - X Daten mit Angaben zu beiden Kontexten
</div>

<div class="fragment" data-fragment-index="3">
- Abbildung der Primärphänomene auf kontinentalem Maßstab
</div>

## 

<section data-background-video="/home/clemens/neomod/neomod_datapool/bronze_age/output.mp4" 
              data-background-color="#f1f1f1">
</section>

## Räumliche Gliederung und zeitliche Entwicklung

<div id="left">
  <figure>
    <img src="/home/clemens/neomod/neomod_datapool/bronze_age/disp_map_full.jpeg">
  </figure>
</div>
<div id="right">
  <figure>
    <img src="/home/clemens/neomod/neomod_datapool/bronze_age/disp_map.jpeg">
  </figure>
</div>

## Räumliche Gliederung und zeitliche Entwicklung

<div id="left">
  <figure>
    <img src="/home/clemens/neomod/neomod_datapool/bronze_age/amount_development_regions_cremation_inhumation.jpeg">
  </figure>
</div>
<div id="right">
  <figure>
    <img src="/home/clemens/neomod/neomod_datapool/bronze_age/amount_development_regions_mound_flat.jpeg">
  </figure>
</div>

## Räumliche Gliederung und zeitliche Entwicklung

<div id="left">
  <figure>
    <img src="/home/clemens/neomod/neomod_datapool/bronze_age/proportions_development_regions_cremation_inhumation.jpeg">
  </figure>
</div>
<div id="right">
  <figure>
    <img src="/home/clemens/neomod/neomod_datapool/bronze_age/proportions_development_regions_mound_flat.jpeg">
  </figure>
</div>

## Theoretische Grundlage: Evolutionary Archaeology

**Cultural evolution theory**: 
Die Prozesse der natürlichen Entwicklung von Spezies durch Evolution wirken auch bei der menschlichen Kulturentwicklung.

\

> - **Evolutionismus** (und Sozialdarwinismus) -- Kulturrelativismus -- **Neoevolutionismus** & Multilineare Evolution

\

> - Neue Perspektiven ab den 1970ern
>     - **Evolutionary psychology**: Menschen verhalten sich wie prähistorische Jäger- und Sammler
>     - **Human behavioural ecology**: Menschen optimieren ihr Verhalten auf Reproduktionserfolg
>     - **Dual inheritance theory**: Kultur bildet ein zweites, viel dynamischeres Vererbungssystem, das genetisch determiniertes Verhalten hinter sich lässt.

## Cultural Transmission and Evolution

> - Bestattungsrituale sind **Verhalten/Ideen/Cultural Traits/Meme** und breiten sich in **Raum und Zeit** aus.
> - Bestattungsrituale sind nur eingeschränkt natürlich determiniert und haben geringe Auswirkung auf die Subsistenz: Sie sind **nicht funktional**.
> - Bestattungsrituale leben vor allem im **sozialen Raum** und ihre Ausbreitung ist von sozialen Beziehungen abhängig. 
> - Bestattungsrituale können sowohl **vertikal** als auch **horizontal** tradiert werden. 

\

> - **Modellierung** 
>     - Landschaft: Diachroner Populationsgraph
>     - Entitäten: Ideen (**the meme's eye view**)
>     - Verhalten: Reproduktion durch Expansion (und Kooperation)

## Populationsgraph: Individuen

```{r, out.width = '100%', fig.height = 8}
palette <- colorRampPalette(colors=c("#000000", "#FFFFFF"))

nodes0 <- tibble::tibble(
  id = 1:20, 
  color.background = palette(length(id)),
  color.border = "black",
  label = id,
  borderWidth = 3,
  shape = "circle",
  level = c(1,1,1,1,2,2,2,3,3,3,1,1,3,3,3,4,4,4,4,4)
)
edges0 <- tibble::tibble()

visNetwork(nodes0, edges0) %>%
  visLayout(randomSeed = 23)
```

## Populationsgraph: Reproduktion

```{r, out.width = '100%', fig.height = 8}
edges1 <- tibble::tibble(
  from = c(1, 3, 6, 11, 10),
  to =   c(2, 4, 7, 12, 13),
  label = "❤",
  font.size = 30,
  width = 8
)

visNetwork(nodes0, edges1) %>%
  visLayout(randomSeed = 23)
```

## Populationsgraph: Vertikale, soziale Beziehungen

```{r, out.width = '100%', fig.height = 8}
edges2 <- edges1 %>% rbind(
  tibble::tibble(
    from = c(1,2,1,2 , 3,4, 6,7,6,7, 6, 7, 11,12,11,12,11,12, 10,13,10,13,10,13, 10, 10,13),
    to =   c(5,5,6,6 , 7,7, 8,8,9,9,10,10, 13,13,14,14,15,15, 16,16,17,17,18,18, 19, 20,20),
    label = "*",
    font.size = c(30),
    width = 4
  )
)

visNetwork(nodes0, edges2) %>%
  visHierarchicalLayout() 
```

## Populationsgraph: Vererbung 

```{r, out.width = '100%', fig.height = 8}
nodes1 <- nodes0
nodes1$color.background[c(4, 7, 9, 10, 17, 19)] = "orange"

visNetwork(nodes1, edges2) %>%
  visHierarchicalLayout() 
```

## Populationsgraph: Horizontale, soziale Beziehungen

```{r}
expand.grid.unique <- function(x, y, include.equals=FALSE) {
  x <- unique(x)
  y <- unique(y)
  g <- function(i) {
    z <- setdiff(y, x[seq_len(i-include.equals)])
    if(length(z)) cbind(x[i], z, deparse.level=0)
  }
  do.call(rbind, lapply(seq_along(x), g)) %>%
    as.data.frame() %>%
    return()
}
```

```{r, out.width = '100%', fig.height = 8}
all_relations <- expand.grid(nodes0$id, nodes0$id) %>%
  dplyr::rename(
    from = Var1,
    to = Var2
  ) %>% dplyr::filter(
    from != to
  )

additional_relations <- all_relations %>% 
  dplyr::anti_join(
    edges2,
    by = c("from", "to")
  ) %>%
  dplyr::anti_join(
    edges2 %>% dplyr::rename(to = from, from = to),
    by = c("from", "to")
  ) %>%
  dplyr::mutate(
    label = NA,
    font.size = 30,
    width = 1
  )

edges3 <- edges2 %>% rbind(additional_relations)

edges3 %>%
 igraph::graph_from_data_frame(d = ., vertices = nodes0, directed = FALSE) %>%
  visNetwork::visIgraph(layout = "layout_in_circle", idToLabel = FALSE)
```

## Populationsgraph: Horizontale, soziale Beziehungen

```{r out.width = '100%', fig.height = 8}
# set.seed(123)
# additional_relations_small <- dplyr::sample_n(additional_relations, 30)
additional_relations_small <- tibble::tibble(
  from = c(2,  3, 5, 8,  9, 13, 14, 16, 17, 18, 19),
  to =   c(3, 11, 6, 9, 10, 14, 15, 17, 18, 19, 20),
  label = NA,
  font.size = 30,
  width = 1
)

edges4 <- edges2 %>% rbind(additional_relations_small)

visNetwork(nodes0, edges4) %>%
  visHierarchicalLayout() 
```

## Populationsgraph: Vererbung und Transmission

```{r out.width = '100%', fig.height = 8}
nodes2 <- nodes0
nodes2$color.background[c(4, 11, 12, 13, 14, 7, 6, 8, 9, 10, 16, 17, 18, 19)] <- "orange"

visNetwork(nodes2, edges4) %>%
  visHierarchicalLayout() 
```

## Fragestellungen

## Gliederung der Arbeit
